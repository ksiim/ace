...
// Расширяем тип Participant, добавляя birth_date
...
const TournamentPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Проверяем, осталось ли до турнира больше 90 дней
    const isRegistrationClosedDueToTime = diffDays > 90;
      ...
      <div className={styles.tournamentContainer}>
        <div className={styles.contentWrapper}>
          <div className={styles.tournamentInfo}>
            ...
            <p className={styles.tournamentRegion}>
              <strong>Регион:</strong>{' '}
              {regions.find((region) => region.id === tournament.region_id)?.name || 'Не указан'}
            </p>
            ...
            <p className={styles.tournamentPrice}>
              <strong>Стоимость участия (за человека):</strong>{' '}
            ...
            <p className={styles.tournamentSex}>
              <strong>Пол участников:</strong>{' '}
              {sexes.find((sex) => sex.id === tournament.sex_id)?.name || 'Не указан'}
            </p>
              ...
              {tournament?.type === 'duo' && (
                <div className={styles.modalInput}>
                  ...
                  {partnerData && partnerData.surname && partnerData.name && partnerData.patronymic && (
                    <p>
                      Партнёр: {partnerData.surname} {partnerData.name} {partnerData.patronymic}
                    </p>
                  )}
                </div>
              )}
