...
export const PlanCard: React.FC<PlanCardProps> = ({
  ...
  return (
    <div className={`${styles.card} ${styles.shimmer}`}>
      {isRecommended && (
        <div className={`${styles.isRecommended} ${styles.marquee}`}>
          <span>Выбор пользователейㅤㅤㅤㅤㅤㅤВыбор пользователей</span>
        </div>
      )}
      ...
      <>
        ...
        <div className={`${styles.priceBlock}`}>
          <div className={styles.priceRow}>
            ...
            {/* Отображение скидки (если не первая) */}
            {!isFirst && discount && (
              <div className={styles.discountBadge}>
                -{discount}%
              </div>
            )}
            {/* Если НЕ первая покупка — выводим зачёркнутую цену */}
            {!isFirst && rawPrice && (
              <div className={styles.rawPrice}>
                <s>{rawPrice}₽</s>
              </div>
            )}
          </div>
          ...
          {/* Месячная цена, если есть */}
          {!isFirst && monthlyPrice && (
