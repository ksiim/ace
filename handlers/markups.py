...
buy_subscription_text = """Данный проект требует значительных финансовых вложений, и подписка помогает покрыть часть затрат для его стабильной работы. 

Кроме того, часть ежемесячной выручки выделяется на поддержку нуждающимся юным спортсменам теннисистам😊"""
